<template>
  <div>
    <div style="background:linear-gradient(135deg,var(--primary-dark),var(--primary));padding:50px 0;color:white;text-align:center">
      ...
      <p style="color:rgba(255,255,255,0.8);margin-top:10px">انضم إلى فريق إبداع سوفت</p>
    </div>
          ...
          <form @submit.prevent="submit">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
              <div class="form-group">
                ...
                <input v-model="form.position" class="form-control" placeholder="مطور، محاسب، مبيعات..." />
              </div>
              <div class="form-group">
            ...
            <div class="form-group">
              ...
              <textarea v-model="form.cover_letter" class="form-control" rows="4" placeholder="اكتب نبذة عن نفسك وسبب تقدمك..."></textarea>
            </div>
            <div class="form-group">
              <label class="form-label">السيرة الذاتية (CV) — PDF أو Word</label>
              <input type="file" accept=".pdf,.doc,.docx" @change="onFile" class="form-control" />
            </div>
...
import { ref } from 'vue'
import api from '../../api'
const form = ref({ full_name: '', phone: '', email: '', position: '', experience_years: '', cover_letter: '' })
const cvFile = ref(null)
const submitting = ref(false)
const success = ref('')
const error = ref('')
const onFile = e => { cvFile.value = e.target.files[0] }
async function submit() {
  ...
  try {
    const { data } = await api.post('/apply-job', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
