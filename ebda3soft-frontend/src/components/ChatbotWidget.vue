      ...
      <div v-if="open" class="chatbot-window">
        ...
        <div class="chatbot-header">
          ...
          <div style="display: flex; gap: 4px">
            <button
              @click="mode = 'bot'"
              :style="`border:none;border-radius:6px;padding:4px 10px;font-size:0.75rem;cursor:pointer;font-family:Cairo,sans-serif;${mode === 'bot' ? 'background:white;color:var(--primary)' : 'background:rgba(255,255,255,0.2);color:white'}`"
            >
              🤖 ذكاء
            </button>
            <button
              @click="mode = 'live'"
              :style="`border:none;border-radius:6px;padding:4px 10px;font-size:0.75rem;cursor:pointer;font-family:Cairo,sans-serif;${mode === 'live' ? 'background:white;color:var(--primary)' : 'background:rgba(255,255,255,0.2);color:white'}`"
            >
              👤 مباشر
            </button>
            <button
        ...
        <template v-if="mode === 'bot'">
          <div class="chatbot-messages" ref="msgContainer">
            <div
              ...
              :key="i"
              :class="`chatbot-msg chatbot-msg-${msg.role}`"
            >
        ...
        <template v-else>
          ...
          >
            ...
            <p
              ...
              "
            >
              فريق الدعم متاح من 8 صباحاً حتى 8 مساءً
            </p>

            <a
              :href="`https://wa.me/${whatsapp}?text=مرحباً، أريد الاستفسار عن أنظمة SoftPro - نظامك المستقبلي`"
              target="_blank"
              style="
                width: 100%;
                display: flex;
...
const messages = ref([
  {
    role: "bot",
    text: "أهلاً وسهلاً! 👋 أنا مساعد SoftPro - نظامك المستقبلي. كيف يمكنني مساعدتك؟",
    time: now(),
  },
]);
const suggestions = ref([
  "ما هي أنظمة SoftPro - نظامك المستقبلي؟",
  "كيف أطلب نظاماً؟",
  "أين مكاتبكم؟",
  "كيف أفتح تذكرة دعم؟",
]);
...
async function sendMsg(text = null) {
  ...
  try {
    ...
    messages.value.push({
      role: "bot",
      text: "عذراً، حدث خطأ. يرجى المحاولة لاحقاً.",
      time: now(),
    });
  } finally {
    typing.value = false;
