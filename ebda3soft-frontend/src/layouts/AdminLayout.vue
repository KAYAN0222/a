  ...
  <div class="admin-layout">
    <aside class="admin-sidebar" :class="{ open: sidebarOpen }">
      ...
      <div style="padding:16px 12px;border-top:1px solid rgba(255,255,255,0.08)">
        <button class="btn btn-outline" style="width:100%;color:rgba(255,255,255,0.7);border-color:rgba(255,255,255,0.2)" @click="doLogout">
          🚪 تسجيل الخروج
        </button>
      </div>
    </aside>
    ...
    <div class="admin-main">
      <header class="admin-topbar">
        <div style="display:flex;align-items:center;gap:12px">
          <button class="navbar-toggle" @click="sidebarOpen = !sidebarOpen" style="color:var(--primary)">☰</button>
          <h2 style="font-size:1rem;font-weight:700;color:var(--primary)">{{ pageTitle }}</h2>
        </div>
        <div style="display:flex;align-items:center;gap:12px">
          <span style="font-size:0.9rem;color:var(--text-muted)">👤 {{ auth.userName }}</span>
